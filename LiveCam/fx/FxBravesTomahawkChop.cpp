...
FxBravesTomahawkChop::FxBravesTomahawkChop()
{
	...
	headModel->modelPath = "./assets/fx/diamond_backs/drone/drone2.obj";
	headModel->texturesPaths.fill({ "./assets/fx/diamond_backs/drone/tex/balance_cube.png", "./assets/fx/diamond_backs/drone/tex/drone.jpg" });
	headModel->name = "animated_tomahawk";
	headModel->modelScale = 5.0f;
	headModel->modelShift = { 0, 0, 0 };
...
void FxBravesTomahawkChop::transform(FXModel& face, ExternalRenderParams &externalRenderParams)
{
	...
	for (int i = 0; i < 1; ++i)
	{
		...
		Affine3d rotation(AngleAxisd(additionalRotation, Vector3d(0, 1, 0)));
